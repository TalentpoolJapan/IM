...
func (m *Model) GetUserProfileByUUID(user UserToken) (userinfo UserBasicInfo, err error) {
	...
	if user.UserType == 1 {
		ok, err = m.MySQLDB.SQL("select uuid,nick_name_oversea as full_name_en,nick_name_jp as full_name_ja,logo as avatar from job_seeker_basic_info where uuid=?", user.Uuid).Get(&userinfo)
	}
	if err != nil {
		return
	}
